...
import { card_types, monster_types, link_markers, rarity, spell_colors, trap_colors, CID_BLACK_LUSTER_SOLDIER, spell_types, trap_types } from "./ygo-constant.mjs";
...
export function generate_condition(params, id_list) {
	...
	if (Number.isSafeInteger(params.mention) && card_table.has(params.mention)) {
		...
		arg.$mention = `「${escape_regexp(card_table.get(params.mention).tw_name)}」(?!怪獸|魔法|陷阱|卡片|通常|永續|裝備|速攻|儀式魔法|場地|反擊)`;
	}
